...
func (a *LogrAdapter) Debug(ctx context.Context, msg string, keysAndValues ...any) {
	a.Logger.V(1).Info(msg, a.enrich(ctx, keysAndValues)...)
}

func (a *LogrAdapter) Info(ctx context.Context, msg string, keysAndValues ...any) {
	a.Logger.Info(msg, a.enrich(ctx, keysAndValues)...)
}

func (a *LogrAdapter) Warn(ctx context.Context, msg string, keysAndValues ...any) {
	a.Logger.V(0).Info(msg, a.enrich(ctx, keysAndValues)...)
}

func (a *LogrAdapter) Error(ctx context.Context, msg string, keysAndValues ...any) {
	enriched := a.enrich(ctx, keysAndValues)
	var err error
	remaining := make([]any, 0, len(enriched))
	for i := 0; i+1 < len(enriched); i += 2 {
		if fmt.Sprint(enriched[i]) == "error" {
			if e, ok := enriched[i+1].(error); ok {
				err = e
				continue
			}
		}
		remaining = append(remaining, enriched[i], enriched[i+1])
	}
	a.Logger.Error(err, msg, remaining...)
}

func (a *LogrAdapter) enrich(ctx context.Context, kv []any) []any {
