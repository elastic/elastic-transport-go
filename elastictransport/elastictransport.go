...
type Config struct {
	...
	LeveledLoggerRequestBody   bool
	LeveledLoggerResponseBody  bool
...
func New(cfg Config) (*Client, error) {
	...
	// When a LeveledLogger is configured but no explicit Logger is set,
	// auto-wire round-trip logging through the LeveledLogger.
	if client.leveledLogger != nil && client.logger == nil {
		client.logger = &leveledRoundTripLogger{
			logger:             client.leveledLogger,
			enableRequestBody:  cfg.LeveledLoggerRequestBody,
			enableResponseBody: cfg.LeveledLoggerResponseBody,
		}
	}
