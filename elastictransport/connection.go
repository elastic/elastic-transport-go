...
func (cp *statusConnectionPool) OnFailure(c *Connection) error {
	...
	if c.IsDead {
		if cp.logger != nil {
			cp.logger.Debug("connection already removed", "url", c.URL)
		}
		c.Unlock()
		return nil
	}
	...
	if cp.logger != nil {
		cp.logger.Debug("removing connection", "url", c.URL)
	}
	c.markAsDead()
...
func (cp *statusConnectionPool) resurrect(c *Connection, removeDead bool) error {
	if cp.logger != nil {
		cp.logger.Debug("resurrecting connection", "url", c.URL)
	}
...
func (cp *statusConnectionPool) scheduleResurrect(c *Connection) {
	...
	if cp.logger != nil {
		cp.logger.Debug("scheduling resurrect",
			"url", c.URL,
			"failures", c.Failures,
			"factor", factor,
			"timeout", timeout,
	...
	go func() {
		...
		case <-timer.C:
			...
			if !c.IsDead {
				if cp.logger != nil {
					cp.logger.Debug("connection already resurrected", "url", c.URL)
				}
				return
			}
